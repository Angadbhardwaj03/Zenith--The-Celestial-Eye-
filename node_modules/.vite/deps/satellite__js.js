  ...
  satrecArray.forEach((satrec, index) => {
    ...
    layout.forEach(({ type, offset, size }, field) => {
      if (Object.hasOwn(constants_exports, field)) {
        writer.writeValue(field, offset, type, constants_exports[field], size);
      }
      if (field === "no_unkozai") {
...
async function createSingleThreadRuntime() {
  const { default: createWasmModuleSingleThread } = await import("./base-release-CO5VKP4K.js");
...
async function createMultiThreadRuntime(options) {
  const { default: createWasmModuleMultiThread } = await import("./pthreads-release-J75VN76N.js");
